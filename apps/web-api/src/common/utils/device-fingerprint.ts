...
/**
 * Custom User Agent format for Umbreon Store Mobile App:
 * UmbreonStore/<version> (<platform>; <os_version>; <device_model>)
 *
 * Examples:
 * - Android: UmbreonStore/1.0.0 (Android; 13; Samsung Galaxy S21)
 * - iOS: UmbreonStore/1.0.0 (iOS; 17.0; iPhone 15 Pro)
 */
export const APP_USER_AGENT_PREFIX = 'UmbreonStore/'
export const APP_USER_AGENT_REGEX =
  /^UmbreonStore\/(\d+\.\d+\.\d+)\s*\(([^;]+);\s*([^;]+);\s*([^)]+)\)/i

export interface MobileAppInfo {
  isUmbreonApp: boolean
  appVersion: string
...
/**
 * Parse Umbreon Store mobile app user agent
 */
export function parseUmbreonAppUserAgent(userAgent: string): MobileAppInfo {
  const match = userAgent.match(APP_USER_AGENT_REGEX)
...
/**
 * Check if user agent is from Umbreon Store mobile app
 */
export function isUmbreonMobileApp(userAgent: string): boolean {
  return userAgent.startsWith(APP_USER_AGENT_PREFIX)
}

/**
 * Generate user agent string for Umbreon Store mobile app
 */
export function generateUmbreonAppUserAgent(
  version: string,
  platform: 'Android' | 'iOS',
  osVersion: string,
  deviceModel: string,
): string {
  return `UmbreonStore/${version} (${platform}; ${osVersion}; ${deviceModel})`
}
...
export function parseUserAgent(userAgent: string): ParsedUserAgent {
  // Check if it's Umbreon Store mobile app first
  const appInfo = parseUmbreonAppUserAgent(userAgent)

  if (appInfo.isUmbreonApp) {
    return {
      browser: 'Umbreon Store App',
      browserVersion: appInfo.appVersion,
      os: appInfo.platform,
...
export function getDeviceInfo(deviceId: string, userAgent: string): DeviceInfo {
  ...
  // For Umbreon Store app, include app info and use device model in fingerprint
  if (parsed.isUmbreonApp && parsed.appInfo) {
