...
import { BalanceMutationType, LoginIsFrom, tb } from '@umbreon/db/types'
import type { MetaPaginated, TUser } from 'src/common/types/meta.type'
import { getDeviceInfo, isUmbreonMobileApp } from 'src/common/utils/device-fingerprint'
...
export class UsersService {
  ...
  async getAllSessions(user: TUser, currentAccessToken?: string) {
    ...
    const formattedSessions = sessions.map((session) => {
      ...
      const isMobileApp = isUmbreonMobileApp(session.user_agent)

      // Determine device name
      let deviceName = session.device_name
      if (!deviceName) {
        deviceName = isMobileApp
          ? `Umbreon Store App (${deviceInfo.os})`
