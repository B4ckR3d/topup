...
import { LoginIsFrom, OAuthProvider, UserRegisteredType, UserRole } from '@umbreon/db/types'
...
export class AuthService {
  ...
  async login(data: LoginDto, headers: LoginHeaders) {
    ...
    const deviceName = deviceInfo.isUmbreonApp
      ? `Umbreon Store App (${deviceInfo.appInfo?.deviceModel || deviceInfo.os})`
    ...
    return {
      ...
      data: {
        ...
        device: {
          ...
          is_umbreon_app: deviceInfo.isUmbreonApp,
          login_from: getLoginSource(deviceInfo),
          ...(deviceInfo.isUmbreonApp &&
            deviceInfo.appInfo && {
